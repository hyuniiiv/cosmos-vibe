...
**파동-입자 이중성 (Wave-Particle Duality)** — 구현되기 전의 목표는 순수한 파동입니다.
모든 가능한 해법이 동시에 존재하는 확률 분포이며, 아직 확정된 형태가 없습니다.
각 cosmos는 다른 각도에서의 측정 — 파동을 하나의 구체적인 입자(작동하는 코드)로
붕괴시킵니다. 전략이 다르다는 건 같은 파동을 다른 각도로 측정한다는 뜻입니다.
같은 목표가 cosmos마다 다른 구현을 낳는 이유가 여기 있습니다: 어느 것이 틀린 게
아니라, 각자가 동일한 해법 공간의 다른 단면을 드러냅니다.
