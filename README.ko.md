...
Cosmos Vibe는 양자물리학 개념을 구체적인 개발 신호로 매핑합니다:
...
**얽힘 (Entanglement)** — 에이전트들은 매 구현 단계마다 서로의 인사이트를 읽습니다.
한 cosmos의 발견이 다른 cosmos로 실시간 전파됩니다 — 전략을 합치지 않으면서.

**결어긋남 (Decoherence)** — cosmos가 자신의 전략을 버리고 다른 cosmos를 통째로
복사하면, 독립적인 샘플로서의 가치를 잃습니다. 규칙이 이를 명시적으로 방지합니다:
얽힘은 영향이지 수렴이 아닙니다.

**결정화 (Crystallization)** — 파동함수 붕괴. 충분히 관측했습니다. 하나의 현실을
선택하고 병합합니다.
