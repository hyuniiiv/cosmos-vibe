...
### 다중 스케일 × 다중 레이어 *(v4.1 기준 최종 모양)*

위 다이어그램은 **우주 스케일(cosmos)에서 CLI 레이어**로 동작하는 코어 루프입니다. 동일한 양자 의사결정 프리미티브가 더 큰 스케일과 더 깊은 레이어에서도 펼쳐집니다:

| | ⌨️ Layer 1 — CLI 스킬 | 📝 Layer 2 — YAML DSL *(v2.0)* | 🐍 Layer 3 — Python *(v3.0+)* |
|---|---|---|---|
| 🌍 **거시 (Macro)** *(v1.2+)* | `/cosmos spin`<br/>`/cosmos singularity` | `experiments/*.qa.yaml`<br/>`spin:` / `singularities:` 키 | `CosmosRun.spin`<br/>`CosmosRun.singularities` |
| 🌌 **우주 (Cosmos)** *(v1.0+)* | `/cosmos spawn`<br/>`/cosmos observe`<br/>`/cosmos crystallize` | `experiments/*.qa.yaml`<br/>`strategies:` 블록 + CI/CD 통합 | `ψ`, `entangle`,<br/>`observe`, `measure` |
| 🔬 **미시 (Micro)** *(v4.0+)* | `/cosmos scan` — 코드 스케일 양자 현상<br/>(테스트 결깨짐, 도약 핫스팟 등) | 동일 키 + `scan:` 블록 | 동일 프리미티브 + verifiable<br/>concept-to-code mapping *(v4.1)* |

**얽힘 모드** *(v1.3)*: `--entanglement none|passive|active|strict`. `strict`는 heartbeat 프로토콜로 라이브 통신을 검증 가능한 증거로 만듭니다 (Bell test 유비). 자세한 내용은 본문 하단 [`/cosmos spawn`](#cosmos-spawn) 및 [선언적 실험 — YAML DSL](#선언적-실험--yaml-dsl) 섹션 참조.
...
| `--models` *(v3.3)* | 모델 이름 콤마 구분 목록 (`--strategies` 개수와 일치 필수). cosmos별 모델 할당. 아래 "모델 다양성" 참조 |
...
spawn:                                  # 필수 — 탐색 자체
  ...
  strategies: [token-bucket, sliding-window, fixed-window]
  models: [claude-haiku-4-5, claude-sonnet-4-6, claude-opus-4-7]   # 선택, v3.3+
  entanglement: passive                                              # 선택
```

**Form B — cosmos별 verbose (Form A의 strategies + models 대체, v3.3+):**
...
`/cosmos observe` (v4.0+)가 자동으로 코드 발견사항 + 층간 신호 표면화 (code finding이 있는 파일을 수정한 cosmos = crystallize 검토 후보).

전체 패턴 카탈로그는 [미시 스케일 — `/cosmos scan`](#미시-스케일--cosmos-scan-v40-신규) 참조.
...
⚛️  미시 스케일 (Code)   — /cosmos scan + .quantum/code/findings.jsonl       ← v4.0
```
...
발견사항은 `.quantum/code/findings.jsonl`에 append. `/cosmos observe` (v4.0+)가 자동으로 코드 스케일 섹션 표면화 + **층간 신호** (cosmos가 수정한 파일에 code finding 있으면 crystallize 검토 대상).

Python `from_cosmos()` (v3.3+)도 `CosmosRun.code_findings` + `code_summary()` 노출 — 네 스케일 모두 한 번에 Python 데이터로 접근 가능.

### 4-스케일 아키텍처 — 원래 비전 완성

```
🌌 우주 스케일    (v1.x) — /cosmos spawn — N개 평행 구현
🌍 거시 스케일    (v1.2) — /cosmos spin + /cosmos singularity — 거시 컨텍스트
⚛️ 미시 스케일    (v4.0) — /cosmos scan — 함수/파일/심볼 양자 상태
🧮 수학 레이어    (v3.x) — Python 프리미티브 — 정통 양자역학
```
...
| `psi(states, amplitudes=…)` *(v3.1)* | 같은 생성자, 양자 모드 — 위상 있는 복소 진폭 |
...
**Path B — 양자 연산 (v3.1, v3.2):**

| 프리미티브 | 목적 |
|----------|------|
| `superpose(a, b, weight_a=1, weight_b=1)` *(v3.1)* | 진폭 덧셈 → 간섭 (보강/상쇄) |
| `bell_state(kind)` *(v3.1)* | 최대 얽힘 2-qubit Bell 상태 (`phi+`/`phi-`/`psi+`/`psi-`) |
| `gate(name, *params)` *(v3.2)* | 양자 게이트 구성: `I`, `X`, `Y`, `Z`, `H`, `S`, `T`, `CNOT`, `CZ`, `SWAP`, `Rx`/`Ry`/`Rz` |
| `apply_gate(psi, gate, qubits=[...])` *(v3.2)* | n-qubit 상태의 지정된 큐빗에 게이트 적용 |
| `measure_in_basis(psi, θ_a, θ_b)` *(v3.2)* | 회전 후 측정 — 다중 기저 관측가능량 측정 |
| `chsh_test(psi, n_trials)` *(v3.2)* | CHSH 프로토콜 실행 — 얽힌 상태에서 Bell 부등식 위반 검증 |
| `density(psi)` *(v3.2)* | 순수 파동함수를 밀도 행렬 `|ψ⟩⟨ψ|`로 변환 |
| `decohere(rho, rate)` *(v3.2)* | 비대각 결맞음 지수 감쇠 (결깨짐 모델) |
| `partial_trace(rho, qubit)` *(v3.2)* | 한 큐빗 자취 — 환원 상태가 얽힘 노출 |

**Layer 1 ↔ Layer 3 interop (v3.3, v4.0):**

| 프리미티브 | 목적 |
|----------|------|
| `from_cosmos(repo_path, weights="by-insight-count")` *(v3.3)* | `.quantum/` cosmos 출력을 Python `CosmosRun`으로 읽기 |
...
| `CosmosRun.code_findings` / `.code_summary()` *(v4.0)* | `/cosmos scan`의 코드 스케일 발견사항 |
...
`--entanglement strict` 모드(v1.3)는 heartbeat 프로토콜로 *검증 가능한* 라이브 얽힘 추가. 라이브 통신의 *증명*이 필요할 때(레이스 컨디션, 분산 시스템 설계, 컴플라이언스). 대부분의 아키텍처 탐색에는 기본 `passive` 모드(best-effort 라이브 + 상시 사후)가 적절한 균형.
...
skills/                                          # Layer 1 — CLI (Claude Code 스킬)
  ...
  scan/SKILL.md              — /cosmos scan               (v4.0, 미시 스케일)
...
.quantum/                    — 런타임 양자 메모리 (git-ignored)
  ...
  code/findings.jsonl        — 코드 레벨 발견사항    (미시 스케일, v4.0)
```
