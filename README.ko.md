...
전략 A          전략 B                    전략 C
 ...
 writes ──────────► * ◄────────────────── writes
    ...
    └────────────────────────────────────────────┘
         │
         ▼
 /cosmos observe   ← 비파괴 측정 + 7개 양자 신호 매핑
         │
         ├── ⚡ 공명 (Resonance)        — N개 cosmos가 독립적으로 동의 → ship
         ├── 🌀 불확정성 (Uncertainty)  — 전략 분기 → 개발자가 선택
         ├── ♊ 축퇴 (Degeneracy)       — 다른 전략 → 동일 구현 → 자연 유일해
         ├── ⚠️ 결깨짐 (Decoherence)    — cosmos가 자기 전략을 상실 → 표본 무효
         ├── ⚛️ 터널링 (Tunneling)     — 가정한 제약이 우회됨 → 검토
         ├── ⚡ 점프 (Quantum Jump)    — 단일 얽힘 읽기로 불연속 전환 → 추적
         └── 🧊 BEC (응축)             — 불확정성 0 + 공명 ≥3 + 전원 참여 → 결정론
         │
         ▼
...
위 다이어그램은 **우주 스케일(cosmos)에서 CLI 레이어**로 동작하는 코어 루프입니다. 동일한 양자 의사결정 프리미티브가 더 큰 스케일과 더 깊은 레이어에서도 펼쳐집니다:

| | ⌨️ Layer 1 — CLI 스킬 | 📝 Layer 2 — YAML DSL | 🐍 Layer 3 — Python |
|---|---|---|---|
| 🌍 **거시 (Macro)** | `/cosmos spin`<br/>`/cosmos singularity` | `experiments/*.qa.yaml`<br/>`spin:` / `singularities:` 키 | `CosmosRun.spin`<br/>`CosmosRun.singularities` |
| 🌌 **우주 (Cosmos)** | `/cosmos spawn`<br/>`/cosmos observe`<br/>`/cosmos crystallize` | `experiments/*.qa.yaml`<br/>`strategies:` 블록 + CI/CD 통합 | `ψ`, `entangle`,<br/>`observe`, `measure` |
| 🔬 **미시 (Micro)** | `/cosmos scan` — 코드 스케일 양자 현상<br/>(테스트 결깨짐, 도약 핫스팟 등) | 동일 키 + `scan:` 블록 | 동일 프리미티브 + verifiable<br/>concept-to-code mapping |

**얽힘 모드**: `--entanglement none|passive|active|strict`. `strict`는 heartbeat 프로토콜로 라이브 통신을 검증 가능한 증거로 만듭니다 (Bell test 유비). 자세한 내용은 본문 하단 [`/cosmos spawn`](#cosmos-spawn) 및 [선언적 실험 — YAML DSL](#선언적-실험--yaml-dsl) 섹션 참조.
...
⚡ Resonance — 신뢰하고 배포 (전략 수렴) ━━━━━━━━━━━━━━━━━━━━━━━━
   "밀리초 정밀도면 충분" — 3개 cosmos 독립 수렴

🌀 Uncertainty — 개발자 선택 (전략 분기) ━━━━━━━━━━━━━━━━━━━━━━━
   자료구조: sorted-set (alpha) | ring-buffer (beta) | hash (gamma)

♊ Degeneracy — 자연 유일해 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   cosmos:alpha 와 cosmos:gamma 가 다른 전략에도 동일한 atomic-counter 구현 도달

⚛️ Quantum Tunneling — 가정한 제약 우회 ━━━━━━━━━━━━━━━━━━━━━━━
   cosmos:alpha: "Redis sorted sets로 별도 rate-limit 테이블 자체가 불필요"

⚡ Quantum Jump — 단일 인사이트로 불연속 전환 ━━━━━━━━━━━━━━━━━━━
   cosmos:beta ← alpha: 구현 중 event-sourcing 으로 전환

⚠️ Decoherence — 표본 무효 위험 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   (이번 실행에서는 감지되지 않음)
...
🚧 미해결 차단 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   cosmos:gamma: 버스트 부하 시 경계 초 이중 카운트

🧊 Bose-Einstein Condensate: (조건 미충족 — Uncertainty 1건 존재)
...
- **Claude Code** (또는 INTEGRATIONS.md 표에 있는 호환 에이전트) — `/plugin` 명령이 동작하는 환경
- **git ≥ 2.20** — cosmos는 `git worktree`로 격리됩니다. `git worktree --help` 가 동작하면 OK
- **POSIX 셸** (bash/zsh) — 번들 스크립트와 일부 스킬이 사용. Windows는 WSL/Git Bash 권장
- **GitHub 접근** — 설치 시 플러그인을 clone. SSH 키가 없다면 아래 [설치 트러블슈팅](#설치-트러블슈팅) 참조
...
**`git@github.com: Permission denied (publickey)` 발생 시**

Claude Code는 기본적으로 SSH로 플러그인을 clone합니다. GitHub SSH 키가 없다면 SSH URL을 HTTPS로 재작성:

```bash
git config --global url."https://github.com/".insteadOf git@github.com:
```

그 뒤 `/plugin install cosmos@quantum-agent` 재시도.

대안:
- `gh auth login` → `gh auth setup-git` (HTTPS + credential helper 사용)
- SSH 키 등록: `ssh-keygen -t ed25519 -C "you@example.com"` 후 GitHub → Settings → SSH Keys에 공개키 등록

**`/cosmos` 명령이 안 보일 때** — `/reload-plugins` 한 번 더 실행 또는 Claude Code 재시작.

**`.quantum/` 디렉터리는 자동 생성됩니다** — 첫 `/cosmos:spawn` 시 repo 루트에 만들어지며, `.gitignore`에 추가됨. 정리는 `/cosmos:stop` (worktree·브랜치 제거) 또는 디렉터리 직접 삭제.
...
## Hello, world — 60초 첫 실행

본격 예제로 넘어가기 전에 단일 cosmos로 감을 잡아봅니다. 아무 git repo 안에서:

```
/cosmos spawn --goal "이 repo의 README를 한 줄로 요약" \
  --strategies "extract-headings"
```

cosmos 1개가 격리된 worktree에서 시작하고, 요약을 만들면서 `.quantum/extract-headings/insights.jsonl`에 발견 사항을 기록합니다.

```
/cosmos observe
```

→ 무엇이 기록됐는지 비파괴적으로 확인 (실행 중에도 OK).

```
/cosmos crystallize extract-headings
```

→ 결과물을 main 브랜치로 끌어와 머지. 끝나면:

```
/cosmos stop
```

→ worktree와 브랜치 정리. 이게 전체 라이프사이클입니다 — **spawn → observe → crystallize → stop**.
단일 cosmos는 빠르고 안전한 워밍업이며, 진짜 가치는 다음 섹션처럼 **여러 cosmos를 병렬 spawn**할 때 나옵니다.
...
`/cosmos spawn` 실행 시 브라우저에서 http://localhost:3141 이 자동으로 열립니다.
...
| 커맨드 | 한 줄 요약 | 언제 쓰나 |
|--------|-----------|----------|
| [`/cosmos spawn`](#cosmos-spawn) | 병렬 코스모스 시작 | 탐색 시작 — 전략마다 하나의 에이전트 |
| [`/cosmos observe`](#cosmos-observe) | 진행 상태 비파괴 조회 | 중간 확인 — 공명/불확정성 맵 보기 |
| [`/cosmos crystallize`](#cosmos-crystallize-id) | 최선 코스모스를 결과물로 수렴 | 탐색 완료 후 — 브랜치 병합 |
| [`/cosmos stop`](#cosmos-stop) | 모든 worktree·브랜치 정리 | 완전 종료 |
| [`/cosmos spin`](#cosmos-spin) | 프로젝트 불변 정체성 선언 | 제약을 모든 spawn에 자동 주입 |
| [`/cosmos singularity`](#cosmos-singularity) | 프로젝트 레벨 이벤트 선언 | 마이그레이션·패러다임 전환 시 이전 인사이트 무효화 |
| [`/cosmos run`](#cosmos-run) | YAML 실험 파일 실행 | 재현 가능·CI/CD 호환 자동화 |
| [`/cosmos scan`](#cosmos-scan) | 코드베이스 양자 현상 스캔 | 타입 우회·미테스트 파일·feature flag 탐지 |
...
flowchart TD
    ...
    SC --> OB
    CR --> ST
    RU -.->|"spin + singularity + spawn 일괄 실행"| SP
...
```
/cosmos spawn --goal "<목표>" --strategies "<s1,s2,...>" [--entanglement <mode>] [--models <m1,m2,m3>]
```
...
| `--entanglement` | `none` / `passive` *(기본값)* / `active` / `strict` — 아래 참조 |
| `--models` | 모델 이름 콤마 구분 목록 (`--strategies` 개수와 일치 필수). cosmos별 모델 할당. 아래 "모델 다양성" 참조 |

**모델 다양성**:

```bash
/cosmos spawn \
  --goal "rate limiting 구현" \
  --strategies "token-bucket,sliding-window,fixed-window" \
...
| 모드 | 동작 |
|------|------|
| `--models` 생략 | 모든 cosmos가 호스트 기본 모델 사용 |
| `--models` 지정 | 길이가 strategies와 일치해야 함. 각 cosmos가 할당된 모델 사용 |

왜 모델을 섞나?
- **비대칭 토큰 경제** — 탐색 fan-out에는 저렴한 모델, 종합 단계에는 비싼 모델
- **블라인드 스팟 완화** — 다른 모델이 *공유 편향*을 줄여 Resonance가 단일 모델 편향에서 오는 *거짓 합의*가 되는 걸 방지

흔한 모델 식별자: `claude-haiku-4-5` (빠름/저렴), `claude-sonnet-4-6` (균형, 기본), `claude-opus-4-7` (최강). 정확한 이름은 Claude Code 설치에 따라 다름.
...
| `strict` | Heartbeat 프로토콜 — 매 단계마다 `heartbeat` 발행 + 다른 cosmos의 미응답 heartbeat에 `heartbeat-ack` 작성 필수. 검증 가능한 라이브 얽힘. | 라이브 통신의 *증명*이 필요할 때 — 레이스 컨디션 디버깅, 분산 시스템 설계, 컴플라이언스 감사 |
...
YAML 하나에 프로젝트 스핀, 특이점, spawn 구성을 모두 선언 가능. **두 spawn 형식** 지원 — 하나 선택:

**Form A — strategies + 선택적 models (병렬 리스트):**
...
spawn:                                  # 필수 — 탐색 자체
  ...
  strategies: [token-bucket, sliding-window, fixed-window]
  models: [claude-haiku-4-5, claude-sonnet-4-6, claude-opus-4-7]   # 선택
  entanglement: passive                                              # 선택
```

**Form B — cosmos별 verbose (Form A의 strategies + models 대체):**
...
Form B는 cosmos마다 개별 구성이 필요할 때 유연. Form A는 대부분 기본값 공유할 때 간결.
...
코드베이스에서 **코드 레벨 양자 현상** 스캔. QuantumAgent의 미시 스케일 — 함수/파일/심볼 수준 양자 상태. 발견은 `.quantum/code/findings.jsonl`에 append.

4가지 검출 타입:

| 타입 | 검출 대상 |
|------|----------|
| `code-tunnel` | 타입 시스템 우회: `as unknown as`, `@ts-ignore`, `# type: ignore`, `eval`, `unsafe`, `transmute` |
| `code-decoherence` | 대응 테스트 없는 소스 파일 (테스트로 "관측"되지 않음) |
| `code-superposition` | feature flag + A/B 분기 (코드가 여러 상태로 실행) |
| `code-jump` *(선택)* | 최근 git churn 임계 초과 (재구조화된 파일) |

언어 지원: TypeScript, JavaScript, Python, Go, Rust.
...
🔬 Code scan complete
   ...
   저장: .quantum/code/findings.jsonl
```

`/cosmos observe`가 자동으로 코드 발견사항 + 층간 신호 표면화 (code finding이 있는 파일을 수정한 cosmos = crystallize 검토 후보).

전체 패턴 카탈로그는 [미시 스케일 — `/cosmos scan`](#미시-스케일--cosmos-scan) 참조.
...
🌍 거시 스케일 (Project) — /cosmos singularity + .quantum/project/spin.json
⚛️  미시 스케일 (Code)   — /cosmos scan + .quantum/code/findings.jsonl
```
...
QuantumAgent 다중 스케일 아키텍처의 세 번째이자 마지막 스케일: **코드 레벨 양자 현상**. `/cosmos spawn`이 우주 스케일(평행 구현), `/cosmos singularity`가 거시 스케일(프로젝트 이벤트)에서 작동한다면, `/cosmos scan`은 함수/파일/심볼 스케일에서 작동.
...
| 타입 | 양자 유비 | 검출 대상 |
|------|----------|----------|
| **`code-tunnel`** | 입자가 고전 금지 장벽 통과 | 타입 시스템 우회: `as unknown as`, `@ts-ignore`, `# type: ignore`, `eval`, dynamic property, `unsafe`, `transmute` |
| **`code-decoherence`** | 환경 결합으로 위상 정보 손실 | 테스트 없는 소스 파일 — 테스트로 "관측"되지 않은 상태 |
| **`code-superposition`** | 시스템이 여러 상태로 동시 존재 | feature flag + A/B 분기: 환경에 따라 활성 분기 선택 |
| **`code-jump`** *(선택)* | 에너지 준위 불연속 전이 | 최근 git churn 임계 초과 — 최근 재구조화된 파일 |

발견사항은 `.quantum/code/findings.jsonl`에 append. `/cosmos observe`가 자동으로 코드 스케일 섹션 표면화 + **층간 신호** (cosmos가 수정한 파일에 code finding 있으면 crystallize 검토 대상).

Python `from_cosmos()`도 `CosmosRun.code_findings` + `code_summary()` 노출 — 네 스케일 모두 한 번에 Python 데이터로 접근 가능.

### 4-스케일 아키텍처 — 원래 비전 완성

```
🌌 우주 스케일 — /cosmos spawn — N개 평행 구현
🌍 거시 스케일 — /cosmos spin + /cosmos singularity — 거시 컨텍스트
⚛️ 미시 스케일 — /cosmos scan — 함수/파일/심볼 양자 상태
🧮 수학 레이어 — Python 프리미티브 — 정통 양자역학
```

Layer 1 (CLI) × 4 스케일 매트릭스 완성. Layer 2 (YAML DSL)와 Layer 3 (Python)는 스케일 전반에 추가적으로 확장.

---

## 선언적 실험 — YAML DSL

QuantumAgent는 CLI 위에 **선언적 레이어**를 추가. 실험이 YAML 파일이 됨 — 버전 관리, 리뷰 가능, 재실행 가능.
...
## Python 프리미티브 — Layer 3

QuantumAgent는 **프로그래머블 레이어**를 추가: 양자 의사결정 프리미티브를 코드에서 직접 노출하는 Python 패키지. Layer 1 (CLI)과 Layer 2 (YAML DSL)는 AI 에이전트 오케스트레이션, Layer 3은 그 아래 깔린 수학을 노출.
...
| 프리미티브 | 목적 |
|----------|------|
| `psi(states, weights=…)` | 의사결정을 확률 분포로 선언 (별칭 `ψ`) — 고전 모드 |
| `psi(states, amplitudes=…)` | 같은 생성자, 양자 모드 — 위상 있는 복소 진폭 |
| `observe(psi)` | 붕괴 없이 분포 읽기 — 비파괴, idempotent |
| `measure(psi, seed=None)` | 본 규칙 샘플링으로 한 상태로 붕괴 — 비가역 |
| `entangle(a, b, correlation)` | 두 결정을 연결, 하나의 측정이 다른 하나를 조건화 |
| `constraint(name, boost=…, suppress=…, where=…) @ psi` | 분포를 휘는 연산자 적용 (고전 모드) |

**Path B — 양자 연산:**

| 프리미티브 | 목적 |
|----------|------|
| `superpose(a, b, weight_a=1, weight_b=1)` | 진폭 덧셈 → 간섭 (보강/상쇄) |
| `bell_state(kind)` | 최대 얽힘 2-qubit Bell 상태 (`phi+`/`phi-`/`psi+`/`psi-`) |
| `gate(name, *params)` | 양자 게이트 구성: `I`, `X`, `Y`, `Z`, `H`, `S`, `T`, `CNOT`, `CZ`, `SWAP`, `Rx`/`Ry`/`Rz` |
| `apply_gate(psi, gate, qubits=[...])` | n-qubit 상태의 지정된 큐빗에 게이트 적용 |
| `measure_in_basis(psi, θ_a, θ_b)` | 회전 후 측정 — 다중 기저 관측가능량 측정 |
| `chsh_test(psi, n_trials)` | CHSH 프로토콜 실행 — 얽힌 상태에서 Bell 부등식 위반 검증 |
| `density(psi)` | 순수 파동함수를 밀도 행렬 `|ψ⟩⟨ψ|`로 변환 |
| `decohere(rho, rate)` | 비대각 결맞음 지수 감쇠 (결깨짐 모델) |
| `partial_trace(rho, qubit)` | 한 큐빗 자취 — 환원 상태가 얽힘 노출 |

**Layer 1 ↔ Layer 3 interop:**

| 프리미티브 | 목적 |
|----------|------|
| `from_cosmos(repo_path, weights="by-insight-count")` | `.quantum/` cosmos 출력을 Python `CosmosRun`으로 읽기 |
| `CosmosRun.psi` | cosmos 이름 위의 Wavefunction (인사이트 수 가중 또는 uniform) |
| `CosmosRun.insights` | cosmos별 파싱된 인사이트 JSONL |
| `CosmosRun.resonance` / `.uncertainty` | 휴리스틱 토큰 중복 신호 (시맨틱 품질은 `/cosmos observe` 사용) |
| `CosmosRun.spin` / `.singularities` | 정의된 경우 프로젝트 스케일 상태 |
| `CosmosRun.code_findings` / `.code_summary()` | `/cosmos scan`의 코드 스케일 발견사항 |

모든 프리미티브 조합 가능 — 예: `cache = constraint("low-latency") @ psi(states, weights=…)`, `entangle(cache, store, ...)`, `measure(cache)`.
...
| 단계 | 상태 | 기능 |
|------|------|------|
| Phase 1 | ✓ 출시 | 복소 진폭, 간섭, Bell 상태 |
| Phase 2 | ✓ 출시 | CHSH 부등식 검정, 다중 기저 측정 |
| Phase 3 | ✓ 출시 | 전체 양자 게이트 라이브러리 (Pauli, Hadamard, CNOT, CZ, SWAP, Rx/Ry/Rz), 회로 합성 |
| Phase 4 | ✓ 출시 | 밀도 행렬, 결깨짐 모델, 부분 자취 |
...
향후 호환 API 설계로 기존 고전 모드 코드는 그대로 작동. 양자 모드는 순수 추가 — `weights=` 대신 `amplitudes=` 전달로 옵트인.

전체 가이드, **실행 가능한 예제 9개** (3개 고전 + Path B 4단계 5개 + cosmos interop 1개), 프리미티브 레퍼런스는 [`python/README.md`](python/README.md) 참조.
...
QuantumAgent의 얽힘은 **두 층**으로 구성됨 — 두 층 다 정직하게 인정하는 것이 시스템이 실제 가치를 산출하는 방식:

| 층 | 메커니즘 | 강도 |
|----|---------|------|
| **라이브 얽힘** | 단계 사이 `.quantum/*/insights.jsonl` 읽기 (best-effort, 프롬프트 기반) | 에이전트 준수도에 의존 — 실행마다 다름 |
| **사후 수렴 탐지** | `/cosmos observe`의 시맨틱 분석으로 모든 인사이트 비교 | **항상 작동** — 라이브 읽기 여부와 무관 |

**사후 수렴이 진짜 가치 엔진.** cosmos가 다른 cosmos의 후기 인사이트를 읽기 전에 끝나도 `/cosmos observe`는 독립 수렴을 Resonance로 표면화. auth-audit 예시가 증명: 3개 cosmos가 JWT 무만료 버그를 *독립적으로* 발견 — 누가 누구를 읽었든 무관하게 그게 resonance.

`--entanglement strict` 모드는 heartbeat 프로토콜로 *검증 가능한* 라이브 얽힘 추가. 라이브 통신의 *증명*이 필요할 때(레이스 컨디션, 분산 시스템 설계, 컴플라이언스). 대부분의 아키텍처 탐색에는 기본 `passive` 모드(best-effort 라이브 + 상시 사후)가 적절한 균형.
...
| **얽힘** | 🔧 | spawn CLAUDE.md 템플릿 — `.quantum/*/insights.jsonl` 필수 읽기 + `--entanglement strict` heartbeat |
...
| **스핀** | 🔧 | `.quantum/project/spin.json` + Spin Preservation 규칙 + `/cosmos spin` |
| **양자 결맞음** | 🪞 | 행동적 — cosmos 실행 내내 전략 보존 |
| **양자 터널링** | 🔧 🔍 | `[TUNNEL]` 인사이트 태그 + `/cosmos scan` 자동 `code-tunnel` 검출 |
...
| **결어긋남** | 🔧 🧮 🔍 | **3 레벨**: observe Decoherence; `decohere()` 밀도 행렬; `code-decoherence` scan |
...
| **등가 원리** *(일반상대성)* | 🪞 | 행동적 — observe의 넓은 시야 + cross-scale 신호 |
...
.quantum/                    — 런타임 양자 메모리 (git-ignored)
  <name>/insights.jsonl      — cosmos별 인사이트 (우주 스케일)
  project/spin.json          — 프로젝트 정체성       (거시 스케일)
  singularities/events.jsonl — 거시 사건             (거시 스케일)
  code/findings.jsonl        — 코드 레벨 발견사항    (미시 스케일)
```
